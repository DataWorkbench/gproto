...
func (t *ElasticSearchSource) Scan(val interface{}) error {
	return json.Unmarshal(val.([]byte), t)
}

// Value for implements driver.Valuer (- database/sql/driver)
func (t *ElasticSearchSource) Value() (driver.Value, error) {
	if t == nil {
		return nil, nil
	}
...
// Scan for implements sql.Scanner (- database/sql).
func (t *ElasticSearchTarget) Scan(val interface{}) error {
	return json.Unmarshal(val.([]byte), t)
}

// Value for implements driver.Valuer (- database/sql/driver)
func (t *ElasticSearchTarget) Value() (driver.Value, error) {
	if t == nil {
		return nil, nil
	}
	return json.Marshal(t)
